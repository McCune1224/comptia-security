...
function presentation(question: QuestionDefinition, rng: () => number): QuestionDefinition {
	...
	if (cloned.kind === 'single-choice' || cloned.kind === 'multiple-choice')
	...
	if (cloned.kind === 'memory')
		cloned.pairs = shuffle(cloned.pairs, rng).map((pair) =>
			rng() < 0.5 ? pair : { ...pair, a: pair.b, b: pair.a }
		);
	if (cloned.kind === 'multi-step')
		cloned.steps = cloned.steps.map((step) => presentation(step, rng)) as typeof cloned.steps;
	return cloned;
}
...
function asView(stored: StoredSession): SessionView {
	...
	return {
		...
		startedAt: stored.summary.started_at,
		...(stored.deadlineAt ? { deadlineAt: stored.deadlineAt } : {}),
		answeredCount: Object.keys(stored.responses).length,
...
function validateResponse(question: QuestionDefinition, response: QuestionResponse): void {
	...
	if (
		...
		if (
			response.optionIds.length !== question.selectCount ||
			!distinct(response.optionIds) ||
			!response.optionIds.every((id) => question.options.some((option) => option.id === id))
		)
			throw new QuizServiceError('INVALID_REQUEST', 'Choice response does not match the question.');
		return;
	}
	if (
		question.kind === 'ordering' &&
		response.kind === 'ordering' &&
		response.itemIds.length === question.items.length &&
		distinct(response.itemIds) &&
		response.itemIds.every((id) => question.items.some((item) => item.id === id))
	)
		return;
	if (
		question.kind === 'matching' &&
		response.kind === 'matching' &&
		Object.keys(response.matches).length === question.premises.length &&
		question.premises.every((premise) => {
			const matchId = response.matches[premise.id];
			return (
				question.targets.some((target) => target.id === matchId) ||
				(question.extraTargets?.some((extra) => extra.id === matchId) ?? false)
			);
		}) &&
		distinct(Object.values(response.matches))
	)
		return;
	if (question.kind === 'numeric' && response.kind === 'numeric' && Number.isFinite(response.value))
		return;
	if (
	...
	if (
		...
		response.lineIds.length === question.selectCount &&
		distinct(response.lineIds) &&
		response.lineIds.every((id) => question.artifact.lines.some((line) => line.id === id))
	)
		return;
	if (
		question.kind === 'configuration' &&
		response.kind === 'configuration' &&
		Object.keys(response.values).length === question.fields.length &&
		question.fields.every((field) =>
			field.options.some((option) => option.id === response.values[field.id])
		)
	)
		return;
	if (
		...
		Object.keys(response.values).length === question.blanks.length &&
		question.blanks.every((blank) => typeof response.values[blank.id] === 'string')
	)
		return;
	if (
		question.kind === 'word-bank' &&
		response.kind === 'word-bank' &&
		Object.keys(response.assignments).length === question.blanks.length &&
		question.blanks.every((blank) =>
			question.bank.some((word) => word.id === response.assignments[blank.id])
		) &&
		new Set(Object.values(response.assignments)).size === question.blanks.length
	)
		return;
	if (
		question.kind === 'sort' &&
		response.kind === 'sort' &&
		Object.keys(response.assignments).length === question.items.length &&
		question.items.every((item) =>
			question.buckets.some((bucket) => bucket.id === response.assignments[item.id])
		)
	)
		return;
	if (
		...
		distinct(response.regionIds) &&
		response.regionIds.every((id) => question.regions.some((region) => region.id === id))
	)
		return;
	if (
		question.kind === 'memory' &&
		response.kind === 'memory' &&
		distinct(response.matchedPairIds) &&
		response.matchedPairIds.every((id) => question.pairs.some((pair) => pair.id === id))
	)
		return;
	if (
		question.kind === 'multi-step' &&
		response.kind === 'multi-step' &&
		response.stepResponses.length === question.steps.length
	) {
		for (let i = 0; i < question.steps.length; i++) {
			validateResponse(question.steps[i], response.stepResponses[i]);
		}
		return;
	}
	throw new QuizServiceError(
		'INVALID_REQUEST',
		'Response does not match the question interaction.'
	);
}
...
export function createQuizService({
	...
	rng = Math.random,
	now = () => new Date(),
	reviewSvc = createReviewService({ repository, bank }),
	examConfig = DEFAULT_EXAM_CONFIG,
	courseSvc = createCourseService({ repository })
}: {
	...
	const expire = (stored: StoredSession): StoredSession => {
		if (
			stored.summary.status === 'active' &&
			stored.deadlineAt &&
			new Date(stored.deadlineAt) <= now()
		) {
			complete(stored);
			return repository.getSession(stored.summary.id)!;
		}
		return stored;
	};
	const requireStored = (id: string) => {
		const stored = repository.getSession(id);
		if (!stored) throw new QuizServiceError('SESSION_NOT_FOUND', 'Session was not found.');
		return expire(stored);
	};
	const complete = (stored: StoredSession): QuizResult => {
		...
		if (stored.summary.status !== 'active')
			throw new QuizServiceError('SESSION_CLOSED', 'Session is not active.');
		...
		} as QuizResult['domainBreakdown'];
		const objectiveBreakdown: QuizResult['objectiveBreakdown'] = {};
		const attemptFactor = (retries: number) =>
		...
		const finalized = repository.complete(
			...
			review.map((item, index) => ({
				index,
				question: stored.questions[index],
				response: item.response,
				points: item.feedback.earnedPoints
			})),
			result.completedAt
		);
		...
		reviewSvc.recordCompletion(
			stored.questions.map((question, index) => ({
				...
				points: review[index].feedback.earnedPoints
			})),
			result.completedAt,
			stored.summary.type
		);
	...
	return {
		...
		saveResponse(id, questionIndex, response, hintUsed = false) {
			...
			if (
				...
				throw new QuizServiceError(
					'RESPONSE_LOCKED',
					'Practice responses are locked after feedback.'
				);
		...
		updateSession(id, update) {
			...
			if (stored.summary.status !== 'active')
				throw new QuizServiceError('SESSION_CLOSED', 'Session is closed.');
			if (
				update.currentIndex !== undefined &&
				(!Number.isInteger(update.currentIndex) ||
					update.currentIndex < 0 ||
					update.currentIndex >= stored.questions.length)
			)
				throw new QuizServiceError('INVALID_REQUEST', 'Question index is out of range.');
			if (
				update.flag &&
				(!Number.isInteger(update.flag.questionIndex) ||
					update.flag.questionIndex < 0 ||
					update.flag.questionIndex >= stored.questions.length)
			)
				throw new QuizServiceError('INVALID_REQUEST', 'Question index is out of range.');
		...
		abandonSession(id) {
			const stored = requireStored(id);
			if (!repository.abandon(id, now().toISOString()))
				throw new QuizServiceError('SESSION_CLOSED', 'Session is closed.');
		},
		completeSession(id) {
			return complete(requireStored(id));
		}
	};
}
