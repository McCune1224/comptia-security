...
function presentation(question: QuestionDefinition, rng: () => number): QuestionDefinition {
	...
	if (cloned.kind === 'memory') cloned.pairs = shuffle(cloned.pairs, rng).map((pair) => (rng() < 0.5 ? pair : { ...pair, a: pair.b, b: pair.a }));
...
function validateResponse(question: QuestionDefinition, response: QuestionResponse): void {
	...
	if (question.kind === 'slider' && response.kind === 'slider' && Number.isFinite(response.value) && response.value >= question.min && response.value <= question.max) return;
	...
	if (question.kind === 'memory' && response.kind === 'memory' && distinct(response.matchedPairIds) && response.matchedPairIds.every((id) => question.pairs.some((pair) => pair.id === id))) return;
