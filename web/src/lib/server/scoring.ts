...
/** Point multiplier when a practice-mode hint was revealed (25% cost). */
export const HINT_POINT_FACTOR = 0.75;

export function scoreQuestion(
	question: QuestionDefinition,
	response: QuestionResponse | null,
	opts?: { hintUsed?: boolean }
): QuestionFeedback {
		...
		switch (question.kind) {
			...
			case 'multiple-choice':
				if (response.kind === 'choice') {
					...
					if (question.format === 'pbq' && question.kind === 'multiple-choice') {
						...
						response.optionIds.length === expected.size &&
						response.optionIds.every((id) => expected.has(id))
					)
						earnedPoints = 1;
				}
				break;
			case 'ordering':
				if (response.kind === 'ordering')
					earnedPoints =
						response.itemIds.filter((id, index) => id === question.correctOrder[index]).length /
						question.correctOrder.length;
				break;
			case 'matching':
				if (response.kind === 'matching')
					earnedPoints =
						question.premises.filter(
							(premise) => response.matches[premise.id] === question.correctMatches[premise.id]
						).length / question.premises.length;
				break;
			case 'numeric':
				if (
					response.kind === 'numeric' &&
					withinTolerance(response.value, question.correctValue, question.tolerance)
				)
					earnedPoints = 1;
				break;
			case 'slider':
			...
			case 'evidence':
				if (response.kind === 'evidence')
					earnedPoints = selectedScore(question.correctLineIds, response.lineIds);
				break;
			case 'configuration':
				if (response.kind === 'configuration')
					earnedPoints =
						question.fields.filter(
							(field) => response.values[field.id] === question.correctValues[field.id]
			...
			case 'hotspot':
				if (response.kind === 'hotspot') {
					const correctIds = question.regions
			...
			case 'memory':
				if (response.kind === 'memory') {
					...
					earnedPoints =
						(response.matchedPairIds ?? []).filter((id) => correctIds.has(id)).length /
						question.pairs.length;
				}
				break;
			case 'multi-step':
				if (response.kind === 'multi-step') {
					const stepScores = question.steps.map((step, i) =>
						scoreQuestion(step, response.stepResponses[i] ?? null)
					);
					earnedPoints =
						stepScores.reduce((sum, s) => sum + s.earnedPoints, 0) / question.steps.length;
				}
				break;
		}
	}
	const rawEarned = earnedPoints;
	if (opts?.hintUsed) earnedPoints *= HINT_POINT_FACTOR;
	const multiSteps =
		question.kind === 'multi-step' && response?.kind === 'multi-step'
			? question.steps.map((step, i) =>
					scoreQuestion(
						step,
						(response as { kind: 'multi-step'; stepResponses: QuestionResponse[] }).stepResponses[
							i
						] ?? null
					)
				)
		...
		...(question.kind === 'single-choice' || question.kind === 'multiple-choice'
			? {
					optionRationales: Object.fromEntries(
						question.options.map((option) => [option.id, option.rationale])
					)
				}
			: {}),
