...
const ALL_OBJECTIVES: ObjectiveId[] = (Object.keys(OBJECTIVES_BY_DOMAIN).map(Number) as Domain[]).flatMap(
	(domain) => OBJECTIVES_BY_DOMAIN[domain] as ObjectiveId[]
);

export function computeMastery(repository: QuizRepository, bank: QuestionBank): MasteryMatrix {
	...
	const objectives: ObjectiveMastery[] = ALL_OBJECTIVES.map((objective) => {
	...
	const domains = (Object.keys(OBJECTIVES_BY_DOMAIN).map(Number) as Domain[]).map((domain) => {
		const items = objectives.filter((item) => item.domain === domain);
	...
	return {
		objectives,
		domains,
		totalAttempted: objectives.reduce((sum, item) => sum + item.attempted, 0)
	};
}
